$(function () {
    ...
    $("table").on('click', "button", function () {
        ...
        if ($(this).text() == "修改") {
            ...
            //depID, depParentID, depName, depHeader, depPhone, remark

            //var inputs = $("#editPersonModalBody").find('input');
            $("#editDepID").val(trObj.children(":nth-child(1)").text());
            $("#editDepParentID").val(trObj.children(":nth-child(2)").text());
            $("#editDepName").val(trObj.children(":nth-child(3)").text());
            $("#editDepHeader").val(trObj.children(":nth-child(4)").text());
            $("#editDepPhone").val(trObj.children(":nth-child(5)").text());
            $("#editRemark").val(trObj.children(":nth-child(6)").text());
            ...
            //$("#editDepID").val(trObj.children(":nth-child(4)").text()).trigger("change");            //console.log($("#editDepID").val());
    ...
    $("#saveEdit").click(function () {
        ...
        var editinfor = {
            ...
            "action": "editDep",
            "depID": $("#editDepID").val(),
            "depParentID": $("#editDepParentID").val(),
            "depName": $("#editDepName").val(),
            "depHeader": $("#editDepHeader").val(),
            "depPhone": $("#editDepPhone").val(),
    ...
    $('#SaveDelete').click(function () {
        //depID, depParentID, depName, depHeader, depPhone, remark
        var depID = delEditButton.parent().parent().children(":nth-child(1)").text();
    ...
    $("#saveAdd").on('click', function () {
        ...
        ////depID, depParentID, depName, depHeader, depPhone, remark
        var depID = $("#addDepID").val();
        var depName = $("#addDepName").val();

        if (depID == "" || depName == "") {
            alert("请将部门名称或部门ID信息填写完整！");
            return;
        }
        //下面的代码ajax修改的用户json数据
        var addinfor = {
            "action": "addDep",
            "depID": $("#addDepID").val(),
            "depParentID": $("#addDepParentID").val(),
            "depName": $("#addDepName").val(),
            "depHeader": $("#addDepHeader").val(),
            "depPhone": $("#addDepPhone").val(),
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                //console.log(data["err_message"]);
                if (data["err_message"]) {
                    if (data["err_message"] == "depIsExist") {
                        $('#jinggaomodel').text('部门信息已经存在，无法添加！');
                        $('#myModal').modal({
                            backdrop: false,
                            keyboard: false,
                            show: true

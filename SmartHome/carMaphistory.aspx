  ...
  <ul>
      ...
      <li> <a href="map.aspx"><i class="icon icon-signal"></i> <span>车辆最新定位</span></a> </li>
       <li> <a href="maphistory.aspx"><i class="icon icon-pencil"></i> <span>车辆历史轨迹</span></a> </li>
        <li class="active"> <a href="carMaphistory.aspx"><i class="icon icon-pencil"></i> <span>查看车辆历史轨迹</span></a> </li>
...
<!--breadcrumbs-->
  <div id="content-header">
    <div id="breadcrumb"> <a href="carMaphistory.aspx" title="车辆历史轨迹" class="tip-bottom"><i class="icon-pencil"></i>车辆历史轨迹</a></div>
  </div>
    ...
    <%--horizontal--%>
      ...
      <form action="#" method="get" class="form-horizontal">
           ...
           <div class="form-group">
                ...
                <label class="control-label">请点确定查看历史轨迹：</label>
        ...
        $(function () {
            ...
            function loaddataStartEnd(startdate, enddate) {
                $.ajax({
                    type: "post",                          //提交方式
                    url: "map.ashx",        //一般处理程序的路径
                    data: { name: "startend1", startdatetime: startdate, enddatetime: enddate, platenumber: imei },           //向后台传入的值
                    dataType: "text",                       //返回值格式
                    //dataType: "json",                       //返回值格式
                    success: function (data) {
                        console.log(data);
            ...
            $("#startendBtn").on('click', function () {
                ...
                //}
                loadstimebypntn(trackname, imei);
                loadetimebypntn2(trackname, imei);
                //loadtimebypntn(trackname, imei);
                var startDate = tempstartdatetime;
                var endendDate = tempenddatetime;
                console.log(startDate);
                console.log(endendDate);

                loaddataStartEnd(startDate,endendDate);
            });
            //--------------------------------根据车牌号查询cartrack得到trackname下拉框---------------------------------------//
            ...
            trackSelectObj.change(function () {
                ...
                if (trackSelectObj.val() != "nodata") {
                    //根据车牌号和轨迹名称加载点列表
                    trackname = trackSelectObj.val();
                    //loadstimebypntn(trackname, imei);
                    //loadstimebypntn2(trackname, imei);
            ...
            //--------------------------------根据车牌号和trackname查询cartrack得到时间信息---------------------------------------//
            function loadstimebypntn(trackname, imei) {
                //alert(plateNumber);
                $.ajax({
                    type: "post",                          //提交方式
                    url: "h5marker.ashx",        //一般处理程序的路径
                    data: { action: "loadpointlist", trackName: trackname, plateNumber: imei },           //向后台传入的值
                    dataType: "text",                       //返回值格式
                    //dataType: "json",                       //返回值格式
                    success: function (data) {
                        var schema = eval('(' + data + ')');
                        console.log(schema);
                        tempstartdatetime = schema;
                        console.log(tempstartdatetime);
            ...
            function loadetimebypntn2(trackname, imei) {
                //alert(plateNumber);
                $.ajax({
                    type: "post",                          //提交方式
                    url: "h5marker.ashx",        //一般处理程序的路径
                    data: { action: "loadpointlist2", trackName: trackname, plateNumber: imei },           //向后台传入的值
                    dataType: "text",                       //返回值格式
                    //dataType: "json",                       //返回值格式
                    success: function (data) {
                        var schema = eval('(' + data + ')');
                        console.log(schema);
                        // console.log(data.endDate);
                        tempenddatetime = schema;
                        console.log(tempenddatetime);
